...
class TopologyMapper:
    ...
    def analyze_traffic_patterns(self) -> Dict[str, Any]:
        ...
        # Average latency
        avg_latency = (
            sum(obs.latency_ms for obs in self.traffic_observations) / len(self.traffic_observations)
            if self.traffic_observations else 0.0
        )
        
        # Error rate
        error_count = sum(
            1 for obs in self.traffic_observations
            if obs.status_code and obs.status_code >= 400
        )
        error_rate = error_count / len(self.traffic_observations) if self.traffic_observations else 0.0

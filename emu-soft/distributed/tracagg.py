...
class TraceAggregator:
    ...
    def complete_trace(self, trace_id: str) -> Optional[Trace]:
        ...
        start_times = [datetime.datetime.fromisoformat(s.start_time.replace('Z', '+00:00')) for s in spans]
        end_times = []
        for span in spans:
            if span.end_time:
                end_times.append(datetime.datetime.fromisoformat(span.end_time.replace('Z', '+00:00')))
        
        start_time = min(start_times).isoformat()
        end_time = max(end_times).isoformat() if end_times else start_time
        
        duration_ms = (max(end_times) - min(start_times)).total_seconds() * 1000 if end_times else 0.0
    ...
    def query_traces(
        ...
        for trace in self.completed_traces:
            ...
            if operation_name and trace.root_span and trace.root_span.operation_name != operation_name:
                continue
    ...
    def get_service_statistics(self, service_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if service_name:
            ...
            stats['avg_duration_ms'] = (
                ...
                if stats['span_count'] > 0 else 0.0
            )
        ...
        for svc_name, stats in self.service_stats.items():
            ...
            svc_stats['avg_duration_ms'] = (
                ...
                if svc_stats['span_count'] > 0 else 0.0
            )
...
class TracePerformanceAnalyzer:
    ...
    def analyze_traces(self, traces: List[Trace]) -> Dict[str, Any]:
        ...
        slowest = sorted(
            results['operation_statistics'].items(),
            key=lambda x: x[1]['p95_ms'],
            reverse=True
        )[:10]
        
        results['slowest_operations'] = [
            {'operation': op, **stats} for op, stats in slowest
        ]
    ...
    def _percentile(self, data: List[float], percentile: float) -> float:
        ...
        return sorted_data[floor_index] + (sorted_data[floor_index + 1] - sorted_data[floor_index]) * (index - floor_index)
    
    def identify_bottlenecks(self, trace: Trace, threshold_ms: float = 100.0) -> List[Dict[str, Any]]:
        """
        ...
        for span in trace.spans:
            if span.duration_ms >= threshold_ms:
                bottlenecks.append({
                    ...
                    'percentage_of_trace': (span.duration_ms / trace.duration_ms * 100) if trace.duration_ms > 0 else 0.0
                })

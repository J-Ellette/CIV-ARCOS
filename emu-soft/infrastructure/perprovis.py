...
class PerformanceProfiler:
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self._update_io_profile(operation_type, bytes_transferred, time_ms, error_occurred)
    ...
    def _update_function_profile(
        ...
        with self._lock:
            ...
            self.function_profiles[function_key].update(
                execution_time_ms,
                memory_bytes,
                is_error
            )
    ...
    def _update_call_graph(self, caller_key: str, callee_key: str):
        ...
        with self._lock:
            ...
            if caller_key not in self.call_graph:
                ...
                self.call_graph[caller_key] = CallGraphNode(
                    function_name=func,
                    module_name=module
                )
    ...
    def get_detailed_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'function_profiles': {
                key: profile.to_dict()
                for key, profile in self.function_profiles.items()
            },
            'io_profiles': {
                key: profile.to_dict()
                for key, profile in self.io_profiles.items()
            },
            'call_graph': {
                key: node.to_dict()
    ...
    def print_summary(self):
        ...
        for func in summary['top_functions_by_time']:
            print(f"{func['function']:<50} {func['total_time_ms']:>10.2f}   {func['call_count']:>8,}")
...
class ProfileVisualizer:
    ...
    def generate_call_tree(self, root_function: Optional[str] = None, max_depth: int = 5) -> str:
        ...
        def build_tree(node_key: str, depth: int, prefix: str):
            ...
            for callee_key in sorted(node.callees.keys()):
                call_count = node.callees[callee_key]
                new_prefix = f"{prefix}  "
        ...
        if root_function:
            ...
            roots = [
                key for key, node in self.profiler.call_graph.items()
                if not node.callers
            ]
    ...
    def generate_hotspot_report(self, top_n: int = 20) -> str:
        ...
        sorted_functions = sorted(
            self.profiler.function_profiles.items(),
            key=lambda x: x[1].total_time_ms,
            reverse=True
        )[:top_n]

...
class RiskMapVisualizer:
    ...
    def _create_risk_component(
        ...
        risk_factors = {
            "complexity": self._normalize_score(component_data.get("complexity", 0), 0, 50),
    ...
    def _generate_default_components(self, evidence_data: Dict[str, Any]) -> List[RiskComponent]:
        ...
        risk_score = sum(
            project_risk[factor] * self.risk_weights[factor]
            for factor in project_risk
        )
    ...
    def _calculate_risk_distribution(self, components: List[RiskComponent]) -> Dict[str, int]:
    ...
    def _generate_risk_recommendations(
        ...
        if hotspots:
            ...
            for factor, count in sorted(top_factors.items(), key=lambda x: x[1], reverse=True):
        ...
        if not recommendations:
            recommendations.append("Continue monitoring risk levels and maintain current quality standards")
    ...
    def _generate_html_component_list(self, risk_map: RiskMap) -> str:
        ...
        for level in ["critical", "high", "medium", "low"]:
            ...
            if components:
                html.append(f'<h3>{level.capitalize()} Risk ({len(components)} components)</h3>')

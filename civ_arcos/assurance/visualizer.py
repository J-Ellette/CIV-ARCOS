...
Generates visual representations of argument structures using Drakon Editor.
"""
...
class GSNVisualizer:
    ...
    Uses Drakon Editor for enhanced visualization with better layouts and styling.
    """

    # Node shapes for different GSN types (for fallback DOT generation)
    NODE_SHAPES = {
    ...
    def __init__(self):
        ...
        # Path to Drakon generator
        self.drakon_generator_path = os.path.join(
    ...
    def to_svg(self, case: AssuranceCase) -> str:
        """
        Generate SVG representation of the assurance case using Drakon Editor.

        Args:
            case: AssuranceCase to visualize

        Returns:
            SVG format string generated by Drakon Editor
        """
        try:
            return self._generate_drakon_svg(case)
        except Exception as e:
            print(f"Warning: Drakon generation failed ({e}), falling back to basic SVG")
            return self._generate_basic_svg(case)

    def _generate_drakon_svg(self, case: AssuranceCase) -> str:
        """
        Generate SVG using the Drakon chart generator.
        ...
        """
        # Check if Node.js and Drakon generator exist
        if not os.path.exists(self.drakon_generator_path):
            raise FileNotFoundError("Drakon generator not found")
        
        # Convert case to JSON format for Drakon generator
        gsn_data = {
            "title": case.title,
        ...
        # Convert GSN nodes to Drakon-compatible format
        for node_id, node in case.nodes.items():
            gsn_data["nodes"][node_id] = {
                "id": node_id,
                "node_type": node.node_type.value,
                "title": node.statement[:50] + "..." if len(node.statement) > 50 else node.statement,
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as tmp_input:
            json.dump(gsn_data, tmp_input, indent=2)
            tmp_input_path = tmp_input.name
            
        with tempfile.NamedTemporaryFile(mode='w', suffix='.svg', delete=False) as tmp_output:
            tmp_output_path = tmp_output.name
        
        try:
            # Run Drakon generator
            result = subprocess.run([
                'node', self.drakon_generator_path, tmp_input_path, tmp_output_path
            ], capture_output=True, text=True, timeout=30)
            
            if result.returncode != 0:
                raise RuntimeError(f"Drakon generator failed: {result.stderr}")
